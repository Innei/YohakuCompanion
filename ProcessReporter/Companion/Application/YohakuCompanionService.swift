...
final class YohakuCompanionService: ObservableObject {
    ...
    private let capture: CompanionApplicationPresenceCapture
    private let capturePolicySubscriptions = DisposeBag()
    private var capturePolicyFingerprint: CompanionApplicationPresencePolicyFingerprint
    ...
    private convenience init() {
        self.init(
            ...
            capture: CompanionApplicationPresenceCapture()
        )
    ...
    /// Captures exactly the same privacy-sanitized application-only projection
    /// used by Live Desk. The capture adapter deliberately guarantees media=nil.
    func refreshPreview() {
        let policyChanged = invalidatePreviewIfPolicyChanged()
        captureAndRecordPreview()
        if policyChanged {
    ...
    private func setLiveDeskEnabledTransaction(_ isEnabled: Bool) async throws {
        if isEnabled {
            ...
            let currentSnapshotBeforePersistence = capture.capture()
            ...
            let currentSnapshotAfterPersistence = capture.capture()
    ...
    private func captureAndRecordPreview() {
        recordPreview(capture.capture())
    }
