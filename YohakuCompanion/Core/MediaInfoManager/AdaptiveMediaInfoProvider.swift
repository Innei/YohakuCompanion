...
final class AdaptiveMediaInfoProvider: MediaInfoProvider, @unchecked Sendable {
  ...
  private static func merge(authoritative: MediaInfo, enrichment: MediaInfo) -> MediaInfo {
    ...
    return MediaInfo(
      ...
      // `media-control --now` supplies the current position, while some
      // MediaRemote clients report a non-nil but permanently stale zero.
      elapsedTime: enrichment.elapsedTime ?? authoritative.elapsedTime,
